...
def itemlist():
    ...
    rst.update({"item-count": itemCount, "page": page + 1, "items": items})
...
def addItem():
    ...
    if not CheckArgs.areStr(
        reqJson, ["name", "brief-intro", "md-intro", "thumbnail"]
    ...
    try:
        ...
        if "group" in reqJson:
            if not reqJson["group"]:
                item.group = None
            elif CheckArgs.isStr(reqJson["group"]):
                item.group = reqJson["group"]
            else:
                return CODE_ARG_TYPE_ERR
...
def modifyItem(itemId):
    ...
    try:
        if "name" in itemJson:
            item.name = str(itemJson["name"])
        if "available" in itemJson:
            item.available = bool(itemJson["available"])
        if "rsv-method" in itemJson:
            item.rsvMethod = int(itemJson["rsv-method"])
        if "brief-intro" in itemJson:
            item.briefIntro = itemJson["brief-intro"]
        if "thumbnail" in itemJson:
            item.thumbnail = itemJson["thumbnail"]
        if "md-intro" in itemJson:
            item.mdIntro = itemJson["md-intro"]
        if "attr" in itemJson:
            item.attr = int(itemJson["attr"])
        if "group" in itemJson:
            item.group = itemJson["group"]
    except:
        return CODE_ARG_TYPE_ERR
