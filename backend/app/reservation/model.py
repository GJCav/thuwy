...
class Reservation(db.Model):
    ...
    itemId = db.Column("item_id", SNOWFLAKE_ID)
    guest = db.Column(WECHAT_OPENID, nullable=False)
    reason = db.Column(VARCHAR(4096), nullable=False)
    method = db.Column(INTEGER, nullable=False)
    ...
    examRst = db.Column("exam_rst", TEXT)
    chore = db.Column(VARCHAR(1024 * 8))
    ...
    def hasTimeConflict(self):
        ...
        conflictRsv = (
            db.session.query(Reservation.st, Reservation.ed, Reservation.id)
            .filter(*stateCdn)
    ...
    def fromRsvId(rsvId):
        return (
            db.session.query(Reservation).filter(Reservation.id == rsvId).one_or_none()
        )
...
class LongTimeRsv(SubRsvDelegator):
        ...
        try:
            if code == LongTimeRsv.morningCode:
                st = timestamp.hoursAfter(
                    timestamp.dateToTimestamp(dateStr), LongTimeRsv.morningStartHour
                )
                ed = timestamp.hoursAfter(
                    timestamp.dateToTimestamp(dateStr), LongTimeRsv.morningEndHour
                )
            elif code == LongTimeRsv.afternoonCode:
                st = timestamp.hoursAfter(
                    timestamp.dateToTimestamp(dateStr), LongTimeRsv.afternoonStartHour
                )
                ed = timestamp.hoursAfter(
                    timestamp.dateToTimestamp(dateStr), LongTimeRsv.afternoonEndHour
                )
            elif code == LongTimeRsv.nightCode:
                st = timestamp.hoursAfter(
                    timestamp.dateToTimestamp(dateStr), LongTimeRsv.nightStartHour
                )
                ed = timestamp.hoursAfter(
                    timestamp.dateToTimestamp(dateStr), LongTimeRsv.nightEndHour
                )
                ...
                if timestamp.getWDay(st) != 6:
                    return (
                        None,
                        -1,
                    )  # means dateStr is invalid as it should be a Saturday.
    ...
    def getInterval(rsv: Reservation):
        ...
        for rsvId in Json.loads(rsv.chore)["group-rsv"]["sub-rsvs"]:
            st = (
                db.session.query(Reservation.st)
    ...
    def isBegan(rsv: Reservation, now):
        ...
        if not began:
            ...
            for rsvId in choreJson["group-rsv"]["sub-rsvs"]:
                st, ed = (
                    db.session.query(Reservation.st, Reservation.ed)
                    .filter(Reservation.id == rsvId)
                    .one()
                )
                if st <= now <= ed:
    ...
    def getEndTime(rsv: Reservation):
        ...
        for rsvId in choreJson["group-rsv"]["sub-rsvs"]:
            subRsvEd = (
                db.session.query(Reservation.ed)
                .filter(Reservation.id == rsvId)
                .one()[0]
            )
            ed = max(ed, subRsvEd)
