...
def testAddAdvice():
    ...
    for i in range(adviceCount):
        reqJson = {"title": f"add advice {i}", "content": f"test add advice"}
...
def testResponse():
    for id in adviceIds:
        ...
        reqJson = {"response": f"response {id}"}
...
def testAddAdviceRobustly():
    baseReqJson = {"title": f"test add advice robustly", "content": f"test add advice"}
