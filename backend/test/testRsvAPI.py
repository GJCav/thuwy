...
def _flexInterval(daysAfter: int, st, ed) -> str:
    ...
    interval = "{} {:0>2}:{:0>2}-{:0>2}:{:0>2}".format(
        T.getDate(st), T.getHour(st), T.getMins(st), T.getHour(ed), T.getMins(ed)
    )
    return interval
...
def testRsvFlexTime():
    ...
    for k in fmtErrJson.keys():
        ...
        json = res.json()
        assert (
            json["code"] == CODE_ARG_FORMAT_ERR["code"]
        ), f"测试参数格式错误的情况: {temp}\r\n{json}"
...
def testRsvLongTime():
    ...
    for idx, data in enumerate(testDataSet):
        ...
        assert (
            json["code"] == expCode
        ), f"test: {data}, code: {json['code']}, exp: {expCode}"
...
def testExamRsv():
    ...
    res = R.post(
        url_rsv + str(rsvId) + "/", json={"op": 1, "pass": 1, "reason": "test pass"}
    )
    assert res
    json = res.json()
    assert json["code"] == 0, json
    ...
    res = R.post(
        url_rsv + str(rsvId) + "/", json={"op": 1, "pass": 0, "reason": "test reject"}
    )
    assert res
    json = res.json()
    assert json["code"] == 0, json
    ...
    json = res.json()
    assert json["code"] == 0, json
    assert RsvState.isReject(json["rsv"]["state"]) and RsvState.isComplete(
        json["rsv"]["state"]
    ), json["rsv"]
...
def testCancel():
    ...
    reqJson = {
        ...
        "method": 1,
        "interval": [_da(1) + " 1", _da(1) + " 2", _da(1) + " 3"],
    }
