...
Page({
        ...
        success: (res) => {
          if (res.data.code == 0) {
            ...
            for (var i = 0; i < res.data.rsvs.length; ++i) {
              ...
              if (parseInt(the_rsv.state / 4) % 2 == 1) {
                continue;
              }
  ...
  init(res){ //初始化处理
    ...
    for(var i=0;i<3;++i)
    {
      ctx.moveTo(parseInt(width/4)*i, 0)
      ctx.lineTo(parseInt(width/4)*i,height)
      ctx.stroke()
    }
    ctx.moveTo(width, 0)
    ...
    {
      var t=0
      if(i!=5&&i!=10) t=parseInt(width/4)
      ctx.moveTo(t, i*h+40)
    ...
    ctx.stroke()
    //画横排文字
    for(var i=8;i<24;++i) ctx.fillText((i<10?'0':'')+i+':00',parseInt(width/8)*3,h*(i-8)+50) //时间显示
    ctx.fillText('时间',parseInt(width/8)*3,20)
    ctx.fillText('预约情况',parseInt(width/4)*3,20)
    //画竖排文字
    ctx.font='28px SimSun, Songti SC' //文字样式
    ctx.textAlign='center' //文字横线
    ctx.textBaseline='middle' //文字竖向
    ctx.fillText('上',parseInt(width/8),5*h/3+40)
    ctx.fillText('午',parseInt(width/8),10*h/3+40)
    ctx.fillText('下',parseInt(width/8),20*h/3+40)
    ctx.fillText('午',parseInt(width/8),25*h/3+40)
    ctx.fillText('晚',parseInt(width/8),12*h+40)
    ctx.fillText('上',parseInt(width/8),14*h+40)
    //画出已经预约的时间段
  ...
  touch(e){
    ...
    var touch_x=e.touches[0].pageX-this.data.dis_left;
    var touch_y=e.touches[0].pageY-this.data.dis_top;
    console.log(touch_x,touch_y)
    function change_time(t){
      return ((parseInt(t.slice(0,2))-8)*60+parseInt(t.slice(3)))
    }
    if(touch_x>parseInt(width/2)&&touch_y>40)
    {
      let tmp_data=this.data.occupy[this.data.date_index]
      var begin='08:00'
      var time=(touch_y-40)/(this.data.each_height/60)
      for(var i=0;i<tmp_data.length;++i)
      {
        let tmp_time=tmp_data[i]
