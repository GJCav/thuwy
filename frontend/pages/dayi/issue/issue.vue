<template>
	<view class="col-container" style="align-items: flex-start; padding-bottom: 50rpx; background-color: #fffff9;">
		...
		<view class="bubbles-container">
			<!-- 所有气泡的容器 -->
			<view 
				:class="{'row-container': (bubble.tags.indexOf('#teacher') > -1), 'row-reverse-container':(bubble.tags.indexOf('#student') > -1)}"
				...
				<view style="padding-left: 0rpx; flex-grow: 1;">
					...
					<view
						:class="{'bubble-teacher': (bubble.tags.indexOf('#teacher') > -1), 'bubble-student': (bubble.tags.indexOf('#student') > -1)}"
					>
						<!-- content -->
						{{bubble.content.text}}
					</view>
					<!-- <image v-for="url in " :url="bubble.content.picurl"/> -->
				</view>
			</view> <!-- .bubble -->
		</view>
	</view>
			...
			return {
				...
				bubbles: [{
					...
					},
					{
						...
						root_id: 1,
						content: {
							text: "Duis sed mauris eu urna faucibus rhoncus. Praesent vulputate, nunc id maximus vulputate, metus purus finibus lectus, interdum accumsan dui erat ut odio. Sed vitae nisl egestas lorem varius tincidunt in et urna. Nulla lacinia sodales metus, eget consectetur diam pharetra sed.",
							picurl: ["", ""],
						},
					},
					{
